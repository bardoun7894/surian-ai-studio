...
const ArticleCard: React.FC<ArticleCardProps> = ({ article, variant = 'default' }) => {
    ...
    return (
      <Link href={href} className="group flex items-start gap-4 p-3 rounded-2xl hover:bg-white/5 transition-all cursor-pointer border border-transparent hover:border-white/5">
        ...
        <div className="flex-1 min-w-0">
          ...
          <h4 className="text-sm font-bold text-white dark:text-gov-gold leading-snug mb-2 line-clamp-2 group-hover:text-gov-gold transition-colors">
            {title}
          </h4>
    ...
    return (
      <Link href={href} className="group relative w-full h-full rounded-3xl overflow-hidden border border-white/10 cursor-pointer block">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6">
          <h3 className="text-xl font-bold text-white dark:text-gov-gold mb-2 leading-tight group-hover:text-gov-gold transition-colors">
            {title}
          </h3>
  ...
  // Default
  return (
    <Link href={href} className="group flex flex-col gap-4 p-4 rounded-3xl bg-gov-forest/40 dark:bg-gov-emeraldStatic border border-gov-gold/10 hover:border-gov-gold/30 hover:bg-gov-forest/60 transition-all cursor-pointer h-full">
      ...
      <div className="flex-1 flex flex-col">
        ...
        <h3 className="text-lg font-bold text-white dark:text-gov-gold mb-2 leading-tight group-hover:text-gov-gold transition-colors">
          {title}
        </h3>
