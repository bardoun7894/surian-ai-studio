...
import {
    ShieldAlert, Scale, HeartPulse, BookOpen, GraduationCap, Zap, Droplets, Plane, Wifi, Banknote, Map, Factory, Handshake, Building2, TrendingUp, ShieldCheck, X, ExternalLink
} from 'lucide-react';
...
const GovernmentPartners: React.FC = () => {
    ...
    const iconMap: Record<string, React.ReactNode> = {
        ShieldAlert: <ShieldAlert size={24} />, Scale: <Scale size={24} />, HeartPulse: <HeartPulse size={24} />,
        BookOpen: <BookOpen size={24} />, GraduationCap: <GraduationCap size={24} />, Zap: <Zap size={24} />,
        Droplets: <Droplets size={24} />, Plane: <Plane size={24} />, Wifi: <Wifi size={24} />,
        Banknote: <Banknote size={24} />, Map: <Map size={24} />, Factory: <Factory size={24} />,
        TrendingUp: <TrendingUp size={24} />, ShieldCheck: <ShieldCheck size={24} />
    };
    ...
    // Marquee scroll — useGSAP handles cleanup
    useGSAP(() => {
        ...
        if (selectedDirectorate && modalRef.current && backdropRef.current) {
            ...
            tl.to(backdropRef.current, { opacity: 1, duration: 0.3, ease: 'power2.out' })
                .fromTo(modalRef.current,
                    ...
                    { scale: 1, autoAlpha: 1, y: 0, duration: 0.4, ease: 'back.out(1.2)' },
                    "-=0.2"
                );
            const items = modalRef.current.querySelectorAll('.modal-item');
            if (items.length) {
                tl.fromTo(items, { autoAlpha: 0, x: -10 }, { autoAlpha: 1, x: 0, stagger: 0.05, duration: 0.3 }, "-=0.2");
            }
    ...
    const handleClose = () => {
        if (modalRef.current && backdropRef.current) {
            const tl = gsap.timeline({ onComplete: () => setSelectedDirectorate(null) });
            tl.to(modalRef.current, { scale: 0.95, autoAlpha: 0, duration: 0.2, ease: 'power2.in' })
        ...
        <section className="py-12 md:py-24 bg-white dark:bg-dm-bg border-t border-gray-100 dark:border-gov-border/15 transition-colors">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="text-center mb-10 md:mb-16">
                    <div className="inline-flex items-center gap-1.5 md:gap-2 px-3 py-1.5 md:px-4 md:py-2 bg-gov-gold/10 dark:bg-gov-emerald/20 rounded-full mb-4 md:mb-6">
                        ...
                        <span className="text-gov-gold font-bold text-xs md:text-sm tracking-wide">{t('partners_title')}</span>
                    </div>
                    <h2 className="text-2xl md:text-3xl lg:text-4xl font-display font-bold text-gov-forest dark:text-gov-teal mb-4 md:mb-6">{t('partners_title')}</h2>
                    <p className="text-gov-stone/60 dark:text-white/70 max-w-2xl mx-auto text-sm md:text-lg leading-relaxed">{t('partners_subtitle')}</p>
                </div>

                <div className="relative overflow-hidden w-full group mb-8 md:mb-12 [mask-image:linear-gradient(to_right,transparent,white_5%,white_95%,transparent)]" dir="ltr">
                    <div ref={scrollRef} className="flex gap-4 md:gap-6 w-max flex-nowrap py-2 md:py-4">
                        {[...directorates, ...directorates, ...directorates, ...directorates].map((directorate, index) => (
                            <div key={directorate.id + '-' + index} className="flex-shrink-0" onClick={() => setSelectedDirectorate(directorate)}>
                                <div className="w-36 h-36 md:w-48 md:h-48 lg:w-56 lg:h-56 rounded-2xl md:rounded-3xl bg-white dark:bg-dm-surface/90 border border-gov-gold/10 dark:border-gov-border/20 hover:border-gov-gold/30 dark:hover:border-gov-gold/40 flex flex-col items-center justify-center p-4 md:p-6 transition-all duration-500 hover:shadow-xl md:hover:shadow-2xl hover:-translate-y-2 md:hover:-translate-y-3 cursor-pointer group/card hover:bg-gov-gold/5 dark:hover:bg-white/5 relative overflow-hidden">
                                    <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-br from-white/10 dark:from-white/5 to-transparent pointer-events-none opacity-0 group-hover/card:opacity-100 transition-opacity duration-500"></div>
                                    ...
                                    <h3 className="text-[11px] md:text-sm lg:text-base font-bold text-gov-forest dark:text-gov-teal text-center leading-tight line-clamp-2 px-1 md:px-2">{loc(directorate, 'name')}</h3>
                                    <div className="flex items-center gap-1 md:gap-1.5 mt-2 md:mt-3 opacity-60 group-hover/card:opacity-100 transition-all duration-300">
                                        ...
                                        <span className="text-[9px] md:text-xs font-medium text-gov-sand dark:text-gov-beige">{directorate.servicesCount} {language === 'ar' ? 'خدمة' : 'services'}</span>
                                    </div>
                ...
                <div className="flex justify-center">
                    <Link href="/directorates" className="inline-flex items-center gap-2 px-6 py-3 md:px-10 md:py-4 bg-gov-forest dark:bg-gov-button text-white font-bold rounded-xl md:rounded-2xl hover:shadow-lg hover:shadow-gov-forest/20 dark:hover:shadow-gov-gold/20 transition-all duration-300 group hover:-translate-y-1 active:translate-y-0 text-xs md:text-base">
            ...
            {selectedDirectorate && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-8">
                    <div ref={backdropRef} className="absolute inset-0 bg-black/60 backdrop-blur-sm opacity-0" onClick={handleClose}></div>
                    <div ref={modalRef} className="relative w-full max-w-3xl bg-white dark:bg-dm-surface rounded-[1.5rem] md:rounded-[2rem] shadow-2xl overflow-hidden opacity-0 flex flex-col md:flex-row max-h-[90vh]">
                        <div className="w-full md:w-1/3 bg-gov-forest relative p-6 md:p-8 flex flex-col items-center justify-center text-center">
                            ...
                            <div className="relative z-10">
                                <div className="w-20 h-20 md:w-32 md:h-32 mx-auto mb-4 md:mb-6 bg-white/10 rounded-full flex items-center justify-center backdrop-blur-md border border-white/20 shadow-lg [&>svg]:w-10 [&>svg]:h-10 md:[&>svg]:w-12 md:[&>svg]:h-12">
                                    {iconMap[selectedDirectorate.icon] ? React.cloneElement(iconMap[selectedDirectorate.icon] as React.ReactElement, { className: "text-white" }) : <Building2 className="text-white w-10 h-10 md:w-12 md:h-12" />}
                                </div>
                        ...
                        <div className="flex-1 p-6 md:p-10 overflow-y-auto bg-gray-50 dark:bg-zinc-900/50">
                            <div className="flex justify-between items-start mb-4 md:mb-6">
                                <div>
                                    <h4 className="text-base md:text-lg font-bold text-gov-forest dark:text-gov-teal mb-1 md:mb-2">{language === 'ar' ? 'المديريات والهيئات التابعة' : 'Affiliated Directorates & Bodies'}</h4>
                                    <p className="text-xs md:text-sm text-gov-stone dark:text-white/70">{loc(selectedDirectorate, 'description')}</p>
                                </div>
                                <button onClick={handleClose} className="p-1.5 md:p-2 rounded-full hover:bg-gray-200 dark:hover:bg-white/10 transition-colors">
                            ...
                            {selectedDirectorate.subDirectorates && selectedDirectorate.subDirectorates.length > 0 ? (
                                <div className="grid gap-2 md:gap-3">
                                    {selectedDirectorate.subDirectorates.map((sub) => (
                                        <a key={sub.id} href={sub.isExternal ? (sub.url || '#') : '/directorates/' + selectedDirectorate.id + '/sub-directorates'} target={sub.isExternal ? '_blank' : '_self'} rel="noreferrer" className="modal-item group flex items-center justify-between p-3 md:p-4 bg-white dark:bg-white/5 border border-gov-gold/10 dark:border-white/10 rounded-lg md:rounded-xl hover:border-gov-gold/40 hover:shadow-sm md:hover:shadow-md transition-all duration-300">
                                            <div className="flex items-center gap-2 md:gap-3">
                                                <div className="w-1.5 h-1.5 md:w-2 md:h-2 rounded-full bg-gov-gold group-hover:scale-150 transition-transform"></div>
                                                <span className="font-medium text-[11px] md:text-sm text-gov-forest dark:text-white/70 group-hover:text-gov-gold transition-colors">{loc(sub, 'name')}</span>
                                            </div>
                                ...
                                <div className="text-center py-6 md:py-8 text-xs md:text-sm text-gray-400 italic">{language === 'ar' ? 'لا توجد مديريات فرعية متاحة حالياً' : 'No affiliated directorates available at the moment'}</div>
                            )}
                            <div className="mt-6 md:mt-8 pt-4 md:pt-6 border-t border-gray-200 dark:border-gov-border/15 flex justify-end">
                                <button onClick={handleClose} className="px-4 py-1.5 md:px-6 md:py-2 rounded-md md:rounded-lg bg-gray-200 dark:bg-white/10 text-gov-forest dark:text-white text-xs md:text-sm font-medium hover:bg-gray-300 dark:hover:bg-white/20 transition-colors">{language === 'ar' ? 'إغلاق' : 'Close'}</button>
                            </div>
                        </div>
                    </div>
                </div>
