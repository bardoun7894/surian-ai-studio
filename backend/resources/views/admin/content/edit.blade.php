                ...
                <div class="bg-white dark:bg-surface-dark rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm p-5">
                    ...
                    <div class="flex flex-col gap-4">
                        ...
                        <div>
                            ...
                            @php
                                $currentVideoUrl = $content->metadata['video_url'] ?? null;
                            @endphp
                            ...
                            @if($currentVideoUrl)
                                <div class="mb-3 rounded-lg overflow-hidden border border-slate-200 dark:border-slate-700" id="current-video-preview">
                                    ...
                                    @if(str_contains($currentVideoUrl, 'youtube') || str_contains($currentVideoUrl, 'youtu.be'))
                                        @php
                                            $ytId = '';
                                            if (preg_match('/(?:youtube\.com\/watch\?v=|youtu\.be\/|youtube\.com\/embed\/)([a-zA-Z0-9_-]{11})/', $currentVideoUrl, $m)) {
                                                $ytId = $m[1];
                                            }
                                        @endphp
                                        @if($ytId)
                                            <iframe src="https://www.youtube.com/embed/{{ $ytId }}" class="w-full aspect-video" frameborder="0" allowfullscreen></iframe>
                                        @else
                                            <div class="p-3 text-xs text-slate-500">
                                                <a href="{{ $currentVideoUrl }}" target="_blank" dir="ltr" class="text-primary hover:underline">{{ $currentVideoUrl }}</a>
                                            </div>
                                        @endif
                                    @else
                                        <video controls class="w-full aspect-video bg-black">
                                            <source src="{{ asset($currentVideoUrl) }}" type="video/mp4">
                            ...
                            <div class="flex flex-col gap-3">
                                ...
                                <div>
                                    ...
                                    <input type="url" name="video_url" id="video-url-input" value="{{ old('video_url', $currentVideoUrl && !str_starts_with($currentVideoUrl, '/storage/') ? $currentVideoUrl : '') }}" dir="ltr"
                                        class="w-full rounded-lg border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-primary focus:border-primary text-sm"
