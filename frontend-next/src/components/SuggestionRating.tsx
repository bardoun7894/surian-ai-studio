...
const SuggestionRating: React.FC<SuggestionRatingProps> = ({
    ...
    return (
      ...
      >
        <motion.div
          ...
          className="w-16 h-16 md:w-20 md:h-20 bg-green-100 dark:bg-green-800/30 rounded-full flex items-center justify-center mx-auto mb-3 md:mb-4"
        >
          <CheckCircle className="w-8 h-8 md:w-10 md:h-10 text-green-600 dark:text-green-400" />
        </motion.div>
        <h3 className="text-lg md:text-xl font-bold text-green-800 dark:text-green-300 mb-1 md:mb-2">
          {texts.successTitle}
        </h3>
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-center gap-1.5 md:gap-2 mb-4 md:mb-6">
        {[1, 2, 3, 4, 5].map((star) => (
          <motion.button
            ...
            disabled={isReadOnly}
            className={`p-1.5 md:p-2 transition-colors ${isReadOnly ? 'cursor-default' : 'cursor-pointer'}`}
          >
            <Star
              size={32}
              className={`w-6 h-6 md:w-8 md:h-8 transition-all duration-300 ${star <= (hoverRating || rating)
                  ? 'fill-gov-gold text-gov-gold drop-shadow-md md:drop-shadow-lg'
      ...
      {/* Rating Label */}
      <div className="text-center mb-4 md:mb-6">
        <span className="text-xl md:text-2xl font-bold text-gov-gold">
          {rating > 0 ? `${rating}/5` : '-'}
        </span>
        {rating > 0 && (
          <p className="text-xs md:text-sm text-gray-500 dark:text-white/50 mt-1">
      ...
      {!isReadOnly && !hideHelpfulQuestion && (
        <div className="mb-4 md:mb-6">
          <p className="text-xs md:text-sm font-bold text-gray-700 dark:text-white/80 mb-2 md:mb-3 text-center">
            {texts.helpful}
          </p>
          <div className="flex justify-center gap-2 md:gap-4">
            <motion.button
              ...
              className={`flex items-center gap-1.5 md:gap-2 px-3 py-1.5 md:px-4 md:py-2 rounded-full font-bold transition-all text-xs md:text-base ${feedbackType === 'positive'
            ...
            >
              <ThumbsUp size={18} className="w-3.5 h-3.5 md:w-[18px] md:h-[18px]" />
              {texts.yes}
            </motion.button>
            <motion.button
              whileHover={{ scale: 1.05 }}
              ...
              className={`flex items-center gap-1.5 md:gap-2 px-3 py-1.5 md:px-4 md:py-2 rounded-full font-bold transition-all text-xs md:text-base ${feedbackType === 'negative'
                  ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400 border-2 border-red-500'
                  : 'bg-gray-100 text-gray-600 dark:bg-white/10 dark:text-white/60 hover:bg-gray-200'
                }`}
            >
              <ThumbsDown size={18} className="w-3.5 h-3.5 md:w-[18px] md:h-[18px]" />
              {texts.no}
            </motion.button>
          </div>
        </div>
      ...
      {!isReadOnly && (
        <div className="mb-4 md:mb-6">
          <textarea
            ...
            rows={3}
            className="w-full px-3 py-2 md:px-4 md:py-3 rounded-lg md:rounded-xl border border-gray-200 dark:border-gov-border/15 bg-gray-50 dark:bg-gov-card/10 text-xs md:text-sm resize-none focus:ring-2 focus:ring-gov-gold focus:border-transparent outline-none transition-all"
          />
        </div>
      )}
      ...
      {!isReadOnly && (
        <motion.button
          ...
          className="w-full py-2.5 md:py-3 bg-gov-forest dark:bg-gov-button text-white font-bold rounded-lg md:rounded-xl hover:bg-gov-teal dark:hover:bg-gov-gold transition-colors flex items-center justify-center gap-1.5 md:gap-2 disabled:opacity-50 disabled:cursor-not-allowed text-xs md:text-base"
        >
          {isSubmitting ? (
            <>
              <Loader2 className="animate-spin w-4 h-4 md:w-5 md:h-5" size={20} />
              {texts.submitting}
            </>
          ) : (
            <>
              <Send size={20} className="rtl:-scale-x-100 w-4 h-4 md:w-5 md:h-5" />
              {texts.submit}
            </>
          )}
        </motion.button>
