...
import { OFFICIAL_NEWS, DECREES } from '@/lib/constants';
...
const SearchResultsPage: React.FC<SearchResultsPageProps> = ({ initialQuery = '' }) => {
    ...
    const MOCK_ANNOUNCEMENTS = [
        { id: 'a1', title: 'إعلان مناقصة لتوريد تجهيزات مكتبية', date: '2024-05-20', type: 'tender' },
        { id: 'a2', title: 'مسابقة لتعيين موظفين من الفئة الأولى', date: '2024-05-18', type: 'job' },
        { id: 'a3', title: 'إعلان عن بدء التسجيل في المعهد العالي', date: '2024-05-15', type: 'general' },
    ];
    ...
    const uniqueEntities = Array.from(new Set([
        ...
        'الإدارة العامة للصناعة', 'الإدارة العامة للاقتصاد', 'الإدارة العامة للتجارة الداخلية وحماية المستهلك'
    ])).filter(Boolean);
    ...
    const tabs = [
        { id: 'all', label: language === 'ar' ? 'الكل' : 'All', count: totalResults },
        { id: 'news', label: language === 'ar' ? 'أخبار' : 'News', count: filteredNews.length },
        { id: 'decrees', label: language === 'ar' ? 'مراسيم وقوانين' : 'Decrees & Laws', count: filteredDecrees.length },
        { id: 'announcements', label: language === 'ar' ? 'إعلانات' : 'Announcements', count: filteredAnnouncements.length },
    ];
                    ...
                    {showFilters && (
                        <div className="mt-4 p-6 bg-white dark:bg-white/5 rounded-2xl border border-gray-100 dark:border-white/10 animate-fade-in">
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                <div>
                                    <label className="block text-xs font-bold text-gray-500 uppercase mb-2">من تاريخ</label>
                                    <input
                                        type="date"
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-gray-500 uppercase mb-2">إلى تاريخ</label>
                                    <input
                                        type="date"
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-gray-500 uppercase mb-2">الجهة / التصنيف</label>
                                    <select
                        ...
                        <section>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {filteredNews.map(item => (
                                    <Link key={item.id} href={`/news/${item.id}`} className="block bg-white dark:bg-white/5 p-4 rounded-xl border border-gray-100 dark:border-white/10 hover:border-gov-gold/50 transition-colors cursor-pointer">
                        ...
                        <section>
                            ...
                            <div className="space-y-3">
                                {filteredDecrees.map(item => (
                                    <Link key={item.id} href="/decrees" className="bg-white dark:bg-white/5 p-4 rounded-xl border border-gray-100 dark:border-white/10 hover:border-gov-gold/50 transition-colors flex items-center justify-between cursor-pointer">
                                        <div>
                                            ...
                                            <div className="flex items-center gap-4 mt-1 text-xs text-gray-500 dark:text-white/60">
                                                <span>{language === 'ar' ? `رقم: ${item.number}` : `No: ${item.number}`}</span>
                                                <span>{item.date}</span>
                                            </div>
                                        </div>
                        ...
                        <section>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                {filteredAnnouncements.map(item => (
                                    <Link key={item.id} href="/announcements" className="block bg-white dark:bg-white/5 p-4 rounded-xl border border-gray-100 dark:border-white/10 hover:border-gov-gold/50 transition-colors cursor-pointer">
