...
@section('title', 'إضافة مديرية جديدة')
...
<div class="max-w-2xl mx-auto">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-slate-900 dark:text-white">إضافة مديرية جديدة</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400">تعريف جهة جديدة في الهيكل التنظيمي.</p>
        </div>
    ...
    <div class="bg-white dark:bg-surface-dark rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
        <form action="{{ route('admin.directorates.store') }}" method="POST" class="p-6">
            @csrf
            ...
            <div class="flex flex-col gap-6">
                ...
                <div>
                    ...
                    <textarea name="description" rows="3"
                        class="w-full rounded-lg border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800/50 text-slate-900 dark:text-white focus:ring-primary focus:border-primary placeholder:text-slate-400"
                        placeholder="وصف مختصر لمهام المديرية...">{{ old('description') }}</textarea>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="email" name="email" value="{{ old('email') }}" 
                            class="w-full rounded-lg border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800/50 text-slate-900 dark:text-white focus:ring-primary focus:border-primary placeholder:text-slate-400"
                            placeholder="directorate@example.com">
