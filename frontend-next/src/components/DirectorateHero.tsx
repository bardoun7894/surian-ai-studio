...
const DirectorateHero: React.FC<DirectorateHeroProps> = ({ directorate, hasSubDirectorates }) => {
  ...
  // Main entrance animation — useGSAP handles cleanup
  useGSAP(() => {
    ...
    tl.to(bgPatternRef.current, {
      autoAlpha: 0.1, scale: 1, duration: 2, ease: "power2.out"
    });
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 w-full flex flex-col md:flex-row items-center justify-between gap-8 md:gap-16">
            ...
            <div ref={glassContainerRef} className="absolute inset-0 bg-white/60 dark:bg-dm-surface/90 border border-gov-gold/30 dark:border-gov-gold/15 backdrop-blur-md rounded-[2.5rem] shadow-2xl flex items-center justify-center overflow-hidden transition-all duration-500">
              ...
              <div className="relative w-[95%] h-[95%] flex items-center justify-center">
                {directorate.logo && !imageError ? (
                  <Image id="directorate-logo" src={directorate.logo} alt={loc(directorate, 'name')} fill priority className="absolute inset-0 w-full h-full object-contain drop-shadow-lg hover:scale-105 transition-transform duration-500" onError={() => setImageError(true)} />
                ) : (
                  <Image id="directorate-logo" src="/assets/logo/eagle.png" alt={language === 'ar' ? 'شعار الوزارة' : 'Ministry Emblem'} width={320} height={320} priority className="object-contain drop-shadow-xl hover:scale-105 transition-transform duration-500 opacity-90" />
                )}
              </div>
            </div>
        ...
        <div ref={textContainerRef} className="space-y-2 md:space-y-4 relative z-20 md:w-7/12 md:order-2 text-center md:text-start">
          <div className="flex flex-col items-center md:items-start">
            <div className="animate-text inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/50 dark:bg-gov-brand/10 border border-gov-gold/30 backdrop-blur-sm mb-3 invisible">
              <span className="w-2 h-2 rounded-full bg-gov-gold animate-pulse"></span>
              <span className="text-xs font-bold text-gov-forest dark:text-gov-gold tracking-wide uppercase">{language === 'ar' ? 'مديرية' : 'Directorate'}</span>
            </div>
            <h1 className="animate-text text-2xl md:text-4xl lg:text-5xl font-bold font-display text-gov-forest dark:text-gov-gold mb-3 leading-tight drop-shadow-sm invisible">{loc(directorate, 'name')}</h1>
            <div className="animate-text flex items-center justify-center md:justify-start gap-3 md:gap-5 mt-1 mb-2 md:mb-3 w-full invisible">
              <div className={'h-[1px] w-6 md:w-16 bg-gradient-to-' + (direction === 'rtl' ? 'l' : 'r') + ' from-transparent to-gov-gold'}></div>
              <span className="text-base md:text-xl text-gov-sand dark:text-gov-gold font-display font-bold whitespace-nowrap drop-shadow-sm">{language === 'ar' ? 'وزارة الاقتصاد والصناعة' : 'Ministry of Economy and Industry'}</span>
            ...
            <div className="h-auto overflow-hidden mb-4 relative w-full max-w-lg mx-auto md:mx-0">
              <p className="text-sm md:text-lg text-gov-charcoal/80 dark:text-gov-gold/90 leading-relaxed">{loc(directorate, 'description')}</p>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3 justify-center md:justify-start pt-1 md:pt-4 w-full px-4 md:px-0">
            {hasSubDirectorates && (
              <Link href={'/directorates/' + directorate.id + '/sub-directorates'} className="animate-btn w-full sm:w-auto min-w-[160px] px-6 py-3 bg-gov-teal dark:bg-gov-brand text-white font-bold text-base hover:bg-gov-emerald transition-all shadow-lg hover:shadow-xl hover:-translate-y-1 flex items-center justify-center gap-2 group rounded-xl">
                <Building2 size={18} />
            ...
            <Link href="#services" className="animate-btn w-full sm:w-auto min-w-[160px] px-6 py-3 bg-gov-gold/10 text-gov-forest dark:text-gov-gold border border-gov-gold/30 font-bold text-base hover:bg-gov-gold/20 transition-all flex items-center justify-center gap-2 rounded-xl backdrop-blur-sm">
              <FileCheck size={18} />
          ...
          <div className="grid grid-cols-3 gap-2 sm:gap-4 md:gap-6 w-full max-w-2xl mx-auto mt-3 md:mt-6 border-t border-gov-charcoal/10 dark:border-gov-border/15 pt-3 px-2 md:px-4">
            <div className="animate-stat text-center border-r border-gov-charcoal/10 dark:border-gov-border/15 group cursor-default px-1">
              ...
              <div className="text-base md:text-xl font-display font-bold text-gov-forest dark:text-white tabular-nums mb-0.5">{subDirectoratesCount}</div>
              <div className="text-[9px] md:text-[11px] text-gov-stone dark:text-gov-beige/60 uppercase tracking-widest">{language === 'ar' ? 'مديريات' : 'Sub-Directorates'}</div>
            </div>
            <div className="animate-stat text-center border-r border-gov-charcoal/10 dark:border-gov-border/15 group cursor-default px-1">
              <div className="w-7 h-7 md:w-9 md:h-9 mx-auto rounded-full bg-gov-forest/5 dark:bg-gov-gold/10 flex items-center justify-center text-gov-forest dark:text-gov-gold mb-1 group-hover:bg-gov-forest group-hover:text-white dark:group-hover:bg-gov-gold dark:group-hover:text-gov-forest transition-colors duration-500">
                <FileCheck size={14} className="md:w-[18px] md:h-[18px]" />
              </div>
              <div className="text-base md:text-xl font-display font-bold text-gov-forest dark:text-white tabular-nums mb-0.5">{servicesCount}</div>
              <div className="text-[9px] md:text-[11px] text-gov-stone dark:text-gov-beige/60 uppercase tracking-widest">{language === 'ar' ? 'خدمة' : 'Services'}</div>
            </div>
            <div className="animate-stat text-center group cursor-default">
              <div className="w-7 h-7 md:w-9 md:h-9 mx-auto rounded-full bg-gov-forest/5 dark:bg-gov-gold/10 flex items-center justify-center text-gov-forest dark:text-gov-gold mb-1 group-hover:bg-gov-forest group-hover:text-white dark:group-hover:bg-gov-gold dark:group-hover:text-gov-forest transition-colors duration-500">
                <Newspaper size={14} className="md:w-[18px] md:h-[18px]" />
              </div>
              <div className="text-base md:text-xl font-display font-bold text-gov-forest dark:text-white tabular-nums mb-0.5">{directorate.newsCount || 0}</div>
              <div className="text-[9px] md:text-[11px] text-gov-stone dark:text-gov-beige/60 uppercase tracking-widest">{language === 'ar' ? 'خبر' : 'News'}</div>
            </div>
          </div>
        </div>
      </div>
