...
@section('title', 'تعديل قالب الشكوى - وزارة الاقتصاد')
...
<div class="max-w-3xl mx-auto">
    ...
    <nav class="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400 mb-6">
        ...
        <span class="material-symbols-outlined text-[16px] rtl-flip">chevron_left</span>
        <span class="text-slate-900 dark:text-white font-medium">تعديل القالب</span>
    </nav>

    <!-- Header -->
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-slate-900 dark:text-white">تعديل قالب الشكوى</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400">تحديث بيانات قالب الشكوى</p>
        </div>
    ...
    <div class="bg-white dark:bg-surface-dark rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
        <div class="bg-primary px-6 py-4">
            <h2 class="text-white font-bold text-sm flex items-center gap-2">
                ...
                تعديل بيانات قالب الشكوى
            </h2>
        </div>

        <form action="{{ route('admin.complaint-templates.update', $complaintTemplate) }}" method="POST" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="flex flex-col gap-6">
                ...
                <div class="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-100 dark:border-slate-700">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="name" value="{{ old('name', $complaintTemplate->name) }}" required
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="3"
                            ...
                            placeholder="وصف مختصر للقالب...">{{ old('description', $complaintTemplate->description) }}</textarea>
                ...
                <div class="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-100 dark:border-slate-700">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="name_en" value="{{ old('name_en', $complaintTemplate->name_en) }}" dir="ltr"
                            class="w-full rounded-lg border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-primary focus:border-primary placeholder:text-slate-400"
                    ...
                    <div>
                        ...
                        <textarea name="description_en" rows="3" dir="ltr"
                            class="w-full rounded-lg border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-primary focus:border-primary placeholder:text-slate-400 leading-relaxed"
                            placeholder="Brief description of the template...">{{ old('description_en', $complaintTemplate->description_en) }}</textarea>
                ...
                <div class="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-100 dark:border-slate-700">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <select name="directorate_id"
                                ...
                                @foreach($directorates as $directorate)
                                    <option value="{{ $directorate->id }}" {{ old('directorate_id', $complaintTemplate->directorate_id) == $directorate->id ? 'selected' : '' }}>
                                        {{ $directorate->name_ar }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="text" name="type" value="{{ old('type', $complaintTemplate->type) }}"
                                class="w-full rounded-lg border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-primary focus:border-primary placeholder:text-slate-400"
                        ...
                        <div>
                            ...
                            <input type="number" name="sort_order" value="{{ old('sort_order', $complaintTemplate->sort_order) }}" min="0"
                                class="w-full rounded-lg border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-primary focus:border-primary placeholder:text-slate-400"
                                placeholder="0">
                    ...
                    <div class="flex items-center gap-6 mt-4 pt-4 border-t border-slate-200 dark:border-slate-600">
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" name="requires_identification" value="1" {{ old('requires_identification', $complaintTemplate->requires_identification) ? 'checked' : '' }}
                        ...
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" name="is_active" value="1" {{ old('is_active', $complaintTemplate->is_active) ? 'checked' : '' }}
                                class="h-5 w-5 rounded border-slate-300 text-primary focus:ring-primary">
