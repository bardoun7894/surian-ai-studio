...
const ArticleDetail: React.FC<ArticleDetailProps> = ({
    ...
    const formattedDate = date && !isNaN(new Date(date).getTime())
        ? new Date(date).toLocaleDateString(lang === 'ar' ? 'ar-EG-u-nu-latn' : 'en-US', { year: 'numeric', month: 'long', day: 'numeric' })
        : '';
                ...
                <article className="bg-white dark:bg-gov-card/10 rounded-2xl md:rounded-3xl shadow-xl overflow-hidden border border-gray-100 dark:border-gov-border/15 print:shadow-none print:border-none print:rounded-none print:bg-white">
                    ...
                    <div className="p-4 md:p-12">
                        ...
                        <div className="flex flex-wrap items-center gap-y-3 gap-x-4 text-sm text-gray-500 dark:text-white/70 mb-8 pb-8 border-b border-gray-100 dark:border-gov-border/15">
                            <div className="flex items-center gap-2" title={date && !isNaN(new Date(date).getTime()) ? new Date(date).toLocaleDateString(lang === 'ar' ? 'ar-EG-u-nu-latn' : 'en-US', { year: 'numeric', month: 'long', day: 'numeric' }) : ''}>
                            ...
                            {relatedItems.map((item) => (
                                ...
                                >
                                    ...
                                    <div className="p-5">
                                        ...
                                        <div className="flex items-center gap-2 text-xs text-gray-500" title={item.date && !isNaN(new Date(item.date).getTime()) ? new Date(item.date).toLocaleDateString(lang === 'ar' ? 'ar-EG-u-nu-latn' : 'en-US', { year: 'numeric', month: 'long', day: 'numeric' }) : ''}>
                                            <Calendar size={14} />
