            ...
            <div className="bg-gov-forest text-white py-10 md:py-16 px-4 relative overflow-hidden">
                ...
                <div className="max-w-6xl mx-auto relative z-10 text-center animate-fade-in-up">
                    <div className="w-16 h-16 md:w-24 md:h-24 bg-white/10 rounded-2xl md:rounded-3xl flex items-center justify-center mx-auto mb-4 md:mb-6 backdrop-blur-sm shadow-xl border border-white/20">
                        <Landmark className="text-gov-gold w-8 h-8 md:w-12 md:h-12" />
                    </div>
                    <h1 className="text-2xl md:text-3xl lg:text-4xl font-display font-bold mb-3 md:mb-4 drop-shadow-md">
                ...
                <div className="flex flex-col gap-8">
                    {directorates.slice(0, 3).map((admin, index) => (
                        ...
                        >
                            ...
                            <div className="p-6 md:p-8 bg-gradient-to-r from-gov-beige/50 to-transparent dark:from-white/5 border-b border-gray-100 dark:border-gov-border/15 flex flex-col md:flex-row items-center md:items-start gap-4 md:gap-6 relative">
                                <div className="absolute top-0 bottom-0 right-0 w-2 bg-gov-gold"></div>
                                <div className="w-16 h-16 md:w-20 md:h-20 rounded-2xl bg-gov-forest dark:bg-white/10 flex items-center justify-center text-gov-gold shadow-lg shrink-0">
                                    {getIcon(admin.icon, false)}
                                </div>
                                <div className="text-center md:text-start flex-1">
                                    <h2 className="text-xl md:text-2xl lg:text-3xl font-bold text-gov-charcoal dark:text-white mb-2 md:mb-3">
                                        {getLocalized(admin.name, admin)}
                                    </h2>
                                    <p className="text-gov-stone dark:text-white/70 leading-relaxed max-w-3xl">
                                        {language === 'en' && (admin as any).description_en ? (admin as any).description_en : (admin.description || getLocalized(admin.description, admin))}
                                    </p>
                                </div>
                            ...
                            <div className="p-6 md:p-8">
                                ...
                                {(!admin.subDirectorates || admin.subDirectorates.length === 0) ? (
                                    <p className="text-gov-stone/60 dark:text-white/50 text-sm md:text-base">{language === 'ar' ? 'لا توجد مديريات تابعة.' : 'No sub-directorates found.'}</p>
                                ) : (
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
                                        {admin.subDirectorates.map(sub => (
                                            <Link
                                                ...
                                                className="group p-4 md:p-5 bg-gray-50 dark:bg-white/5 hover:bg-gov-gold/5 dark:hover:bg-gov-gold/10 rounded-2xl border border-gray-100 dark:border-white/10 hover:border-gov-gold/50 transition-all flex items-start gap-3 md:gap-4"
                                            >
                                                <div className="w-8 h-8 md:w-10 md:h-10 rounded-xl bg-white dark:bg-black/20 flex items-center justify-center text-gov-teal dark:text-gov-gold shadow-sm shrink-0">
