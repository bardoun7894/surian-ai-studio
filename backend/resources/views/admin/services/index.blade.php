...
@section('title', 'الخدمات - وزارة الاقتصاد')
...
<div class="max-w-[1440px] mx-auto w-full">
    ...
    <div class="flex flex-wrap items-center gap-2 mb-6 text-sm">
        ...
        <span class="text-primary dark:text-slate-200 font-semibold">الخدمات</span>
    </div>

    <!-- Header & Primary Action -->
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
        <div class="flex flex-col gap-2">
            <h1 class="text-3xl md:text-4xl font-black text-slate-900 dark:text-white tracking-tight">إدارة الخدمات</h1>
            <p class="text-slate-500 dark:text-slate-400 text-base max-w-2xl">إدارة خدمات الوزارة الإلكترونية والتقليدية.</p>
        </div>
        <div class="flex gap-3">
            <a href="{{ route('admin.services.create') }}" class="bg-primary hover:bg-primary-dark text-white font-bold py-2.5 px-5 rounded flex items-center gap-2 shadow-md hover:shadow-lg transition-all">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        ...
        <div class="bg-surface-light dark:bg-surface-dark p-5 rounded border border-slate-200 dark:border-slate-700 shadow-sm relative overflow-hidden group">
            <div class="absolute left-0 top-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity">
                <span class="material-symbols-outlined text-primary text-[64px]">miscellaneous_services</span>
            </div>
            <p class="text-slate-500 dark:text-slate-400 text-sm font-semibold uppercase tracking-wider mb-1">إجمالي الخدمات</p>
            <div class="flex items-baseline gap-2">
                <h3 class="text-2xl font-bold text-slate-900 dark:text-white">{{ number_format($services->total()) }}</h3>
            </div>
        </div>
        ...
        <div class="bg-surface-light dark:bg-surface-dark p-5 rounded border-r-4 border-primary dark:border-primary shadow-sm relative overflow-hidden group">
            ...
            <p class="text-slate-500 dark:text-slate-400 text-sm font-semibold uppercase tracking-wider mb-1">خدمات رقمية</p>
            <div class="flex items-baseline gap-2">
                <h3 class="text-2xl font-bold text-slate-900 dark:text-white">{{ number_format($digitalCount ?? 0) }}</h3>
            </div>
        </div>
        ...
        <div class="bg-surface-light dark:bg-surface-dark p-5 rounded border border-slate-200 dark:border-slate-700 shadow-sm relative overflow-hidden group">
            ...
            <p class="text-slate-500 dark:text-slate-400 text-sm font-semibold uppercase tracking-wider mb-1">خدمات نشطة</p>
            <div class="flex items-baseline gap-2">
                <h3 class="text-2xl font-bold text-slate-900 dark:text-white">{{ number_format($activeCount ?? 0) }}</h3>
            </div>
        </div>
    </div>
    ...
    <div class="bg-surface-light dark:bg-surface-dark rounded-t-lg border border-b-0 border-slate-200 dark:border-slate-700 p-4 flex flex-col xl:flex-row gap-4 justify-between items-start xl:items-center">
        <div class="flex flex-1 w-full xl:w-auto gap-4 flex-col sm:flex-row">
            <form id="filterForm" action="{{ route('admin.services.index') }}" method="GET" class="contents">
                <!-- Search -->
                <div class="relative flex-1 max-w-md group">
                    ...
                    <input name="search" value="{{ request('search') }}" class="w-full bg-background-light dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-lg h-10 pr-10 pl-4 text-sm focus:ring-1 focus:ring-primary focus:border-primary placeholder:text-slate-400 transition-all" placeholder="بحث باسم الخدمة..." type="text"/>
                </div>

                <!-- Status Filter -->
                <div class="flex gap-2 w-full sm:w-auto overflow-x-auto pb-1 sm:pb-0">
                    <div class="relative min-w-[140px]">
                        <select name="status" onchange="this.form.submit()" class="w-full appearance-none bg-background-light dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-lg h-10 pr-3 pl-8 text-sm text-slate-700 dark:text-slate-200 focus:ring-1 focus:ring-primary focus:border-primary cursor-pointer text-start">
                    ...
                    @if(request()->anyFilled(['search', 'status']))
                        <a href="{{ route('admin.services.index') }}" class="h-10 px-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded text-red-600 dark:text-red-400 hover:bg-red-100 transition-colors flex items-center justify-center" title="إلغاء التصفيات">
        ...
        <table class="w-full text-right border-collapse">
            <thead>
                <tr class="bg-primary text-white border-b border-primary-dark">
                    <th class="p-4 text-xs font-bold uppercase tracking-wider text-white/90">الخدمة</th>
                    ...
                    <th class="p-4 text-xs font-bold uppercase tracking-wider text-white/90">الترتيب</th>
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                ...
                <tr class="group hover:bg-primary/5 dark:hover:bg-primary/10 transition-colors">
                    <td class="p-4">
                        <div class="flex items-center gap-3">
                            <div class="size-9 rounded-full bg-primary/10 text-primary flex items-center justify-center text-sm font-bold border border-primary/20">
                                <span class="material-symbols-outlined text-[18px]">{{ $service->icon ?? 'work' }}</span>
                            </div>
                            <div>
                                <p class="text-sm font-bold text-slate-900 dark:text-white">{{ $service->name_ar }}</p>
                                @if($service->name_en)
                                    <p class="text-xs text-slate-500 dark:text-slate-400">{{ $service->name_en }}</p>
                                @endif
                            </div>
                        </div>
                    </td>
                    <td class="p-4">
                        <span class="text-sm text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-700/50 px-2 py-1 rounded">
                            {{ $service->directorate ? $service->directorate->name_ar : '-' }}
                        </span>
                    </td>
                    <td class="p-4">
                        ...
                        @else
                            <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-bold bg-slate-100 text-slate-600 dark:bg-slate-700/50 dark:text-slate-300 border border-slate-200 dark:border-slate-600">
                    ...
                    <td class="p-4">
                        <span class="text-sm font-medium text-slate-700 dark:text-slate-300">{{ $service->display_order }}</span>
                    </td>
                    <td class="p-4 text-center">
                        <div class="flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            <a href="{{ route('admin.services.edit', $service) }}" class="size-8 flex items-center justify-center rounded text-slate-500 hover:text-primary hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
                ...
                @empty
                <tr>
                    <td colspan="6" class="p-8 text-center text-slate-500">لا توجد خدمات.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
