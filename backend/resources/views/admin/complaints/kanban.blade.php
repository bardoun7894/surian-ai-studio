            ...
            <div class="min-w-[320px] max-w-[320px] flex flex-col gap-4 h-full"
                ...
                <div class="kanban-dropzone flex flex-col gap-3 overflow-y-auto pl-2 pb-2 custom-scrollbar min-h-[120px] rounded-xl transition-all duration-200"
                    ...
                    @forelse($grouped[$status] ?? [] as $complaint)
                        <div class="kanban-card bg-white dark:bg-slate-800 p-4 rounded-xl shadow-sm hover:shadow-md transition-all cursor-grab active:cursor-grabbing group border border-slate-200 dark:border-slate-700 select-none"
                             draggable="true"
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    dropzones.forEach(function(zone) {
        zone.addEventListener('dragover', function(e) {
            ...
            // Position indicator: find nearest card to insert before
            if (draggedCard && draggedCard.closest('.kanban-dropzone') !== zone) {
                // Just highlight the zone
            }
        });

        zone.addEventListener('dragleave', function(e) {
            // Only remove highlight when actually leaving the zone
    ...
    function updateComplaintStatus(complaintId, newStatus, card, oldStatus, oldZone) {
        ...
        .catch(function(err) {
            // Revert: move card back to old zone
