...
const NewsSection: React.FC = () => {
  ...
  return (
    <>
    <section className="py-12 md:py-20 bg-gradient-to-b from-gray-50 to-white dark:from-dm-bg dark:to-dm-bg transition-colors" id="news-section">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Header with prev/next + View All */}
        <div className="flex items-end justify-between mb-6 md:mb-10">
          <div>
            <h2 className="text-2xl md:text-3xl font-display font-bold text-gov-charcoal dark:text-gov-teal mb-2">
              {t('news_section_title')}
            </h2>
            <div className="h-1 w-16 bg-gov-gold rounded-full"></div>
          </div>
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="w-9 h-9 rounded-full border border-gov-gold/30 flex items-center justify-center text-gov-gold hover:bg-gov-gold hover:text-white disabled:opacity-30 disabled:cursor-not-allowed transition-all"
            >
            ...
            <Link
              href="/news"
              className="hidden md:inline-flex items-center gap-2 px-5 py-2 bg-white dark:bg-gov-card/10 border border-gov-gold/30 rounded-full text-gov-emerald dark:text-gov-teal text-sm font-bold shadow-sm hover:shadow-md hover:border-gov-gold transition-all"
            >
              {t('view_all')}
        ...
        {loading ? (
          ...
          >
            {allNews.map((item) => (
              <article
                key={item.id}
                className="news-card group flex-shrink-0 w-[75vw] sm:w-[45vw] md:w-[calc(25%-12px)] snap-start bg-white dark:bg-dm-surface rounded-xl overflow-hidden border border-gray-100 dark:border-gov-border/15 shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-300 flex flex-col"
              >
                {item.imageUrl && (
        ...
        <div className="mt-4 text-center md:hidden">
          <Link
            href="/news"
            className="inline-flex items-center gap-2 px-5 py-2 bg-white dark:bg-gov-card/10 border border-gov-gold/30 rounded-full text-gov-emerald dark:text-gov-teal text-sm font-bold"
          >
            {t('view_all')}
            <ArrowRight size={16} className={direction === 'rtl' ? 'rotate-180' : ''} />
          </Link>
