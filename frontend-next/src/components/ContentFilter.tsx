...
export default function ContentFilter({
    ...
    // Determine whether the secondary filters bar should render
    const hasSecondaryBar = showDateFilter || extraFilters || totalCount !== undefined || onSearch;
    ...
    return (
        <div className={`space-y-6 ${className}`}>
            ...
            {/* Secondary Filters Row (Search, Date, Extras, Count) */}
            {hasSecondaryBar && (
                <div className="flex flex-col md:flex-row md:flex-nowrap md:items-center justify-between gap-3 md:gap-4 bg-white dark:bg-gov-card/10 rounded-2xl border border-gray-100 dark:border-gov-border/15 p-2.5 sm:p-4" style={{ scrollbarWidth: "none", msOverflowStyle: "none" }}>

                    {/* Search Input - above filters on mobile, inline on desktop */}
                    {onSearch && (
                        <div className="relative w-full md:w-56 lg:w-72 flex-shrink-0 md:order-last">
                            <input
                                type="text"
                                value={internalSearch}
                                onChange={handleSearch}
                                placeholder={searchPlaceholder || t('search_placeholder')}
                                className="w-full py-2 ltr:pl-9 ltr:pr-4 rtl:pr-9 rtl:pl-4 rounded-xl bg-gray-50 dark:bg-dm-surface border border-gray-200 dark:border-gov-border/25 text-sm text-gov-charcoal dark:text-white focus:outline-none focus:border-gov-teal focus:ring-1 focus:ring-gov-teal/20 transition-all"
                            />
                            ...
                            {internalSearch && (
                                <button
                                    ...
                                    className="absolute right-3 rtl:right-auto rtl:left-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gov-cherry dark:hover:text-red-400"
                                >
                                    <X size={14} />
                                </button>
                            )}
                    ...
                    <div className="flex flex-row flex-wrap md:flex-nowrap items-center justify-between gap-2 sm:gap-4 flex-1 min-w-0 overflow-visible" style={{ scrollbarWidth: "none", msOverflowStyle: "none" }}>
                        ...
                        <div className="flex flex-wrap md:flex-nowrap items-center gap-2 sm:gap-4 min-w-0">
                            ...
                            {extraFilters && (
                                <>
                                    ...
                                    <div className="flex w-auto flex-nowrap items-center gap-2">
                                        {extraFilters}
                                    </div>
                                </>
                            )}
                        ...
                        {totalCount !== undefined && (
                            <div className="text-xs sm:text-sm text-gray-400 dark:text-white/50 font-medium whitespace-nowrap flex-shrink-0">
