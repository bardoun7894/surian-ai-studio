          ...
          <div className="p-8">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {[...Array(6)].map((_, i) => (
                <div
                  key={i}
                  className="p-5 rounded-xl border border-gray-100 dark:border-gov-border/15 bg-gray-50 dark:bg-gov-card/10"
                >
            ...
            <div className="bg-white dark:bg-dm-surface rounded-2xl shadow-sm border border-gray-100 dark:border-gov-border/15 p-6">
              <div className="space-y-4">
                {[...Array(4)].map((_, i) => (
                  <div
                    key={i}
                    className="flex items-center gap-4 p-4 border-b border-gray-100 dark:border-gov-border/15 last:border-0"
                  >
...
export default function DirectorateDetailPage({
  ...
  // Use key={params.id} to force React to fully re-mount DirectorateDetail
  // when navigating between different directorates via client-side navigation.
  // Without this, React reuses the component instance, leading to stale state
  // and inconsistent content depending on navigation path.
  return (
    <div className="min-h-screen flex flex-col bg-gov-beige dark:bg-dm-bg transition-colors duration-500">
      <Navbar />

      <main className="flex-grow">
        <DirectorateDetail key={params.id} directorateId={params.id} />
      </main>
