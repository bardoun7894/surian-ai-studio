    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gov-forest animate-fade-in pt-12 pb-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-12 border-b border-gray-200 dark:border-white/10 pb-6">
          <div>
            <h1 className="text-3xl font-display font-bold text-gov-charcoal dark:text-white mb-2">{t('news_archive_title')}</h1>
            <p className="text-gray-500 dark:text-gray-400">{t('news_archive_desc')}</p>
          </div>
          ...
          <div className="flex items-center gap-2 mt-4 md:mt-0 bg-white dark:bg-white/5 p-1 rounded-lg border border-gray-200 dark:border-white/10">
            {[t('news_filter_all'), t('news_filter_politics'), t('news_filter_economy'), t('news_filter_services'), t('news_filter_decrees')].map(cat => (
              <button
                key={cat}
                onClick={() => setFilter(cat === t('news_filter_all') ? 'all' : cat)}
                className={`px-4 py-2 rounded-md text-sm font-bold transition-colors ${(filter === 'all' && cat === 'الكل') || filter === cat
