      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-12">
          ...
          <div>
            ...
            <ul className="space-y-3 text-sm text-gov-stone dark:text-gov-beige/80 transition-colors">
              ...
              <li><a href="#" onClick={(e) => handleLinkClick(e, 'SITEMAP')} className="hover:text-gov-gold transition-colors flex items-center gap-2 font-bold"><Network size={12} /> {t('site_map')}</a></li>
              <li><a href="#" onClick={(e) => handleLinkClick(e, 'ADMIN_DASHBOARD')} className="hover:text-gov-gold transition-colors flex items-center gap-2 text-xs opacity-50"><span className="w-1 h-1 bg-gov-gold rounded-full"></span> {t('footer_admin_login')}</a></li>
            </ul>
          </div>
          ...
          <div>
            ...
            <div className="flex gap-2">
              <button
                ...
                title={t('accessibility_increase')}
              >
                A+
              </button>
              <button
                ...
                className="px-3 py-1 bg-gray-100 dark:bg-gov-emeraldLight rounded hover:bg-gov-gold hover:text-white dark:hover:text-gov-forest transition-colors text-xs font-bold border border-gov-gold/20"
                title={t('accessibility_decrease')}
              >
                A-
              </button>
              <button
                ...
                className="px-3 py-1 bg-gray-100 dark:bg-gov-emeraldLight rounded hover:bg-gov-gold hover:text-white dark:hover:text-gov-forest transition-colors text-xs font-bold border border-gov-gold/20"
                title={t('accessibility_high_contrast')}
              >
        ...
        <div className="pt-8 border-t border-gov-gold/10 flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          <div className="flex items-center gap-4">
            <img src="/assets/logo/Asset-14@3x.png" className="h-8 w-auto opacity-30 grayscale" alt="logo footer" />
          </div>
        </div>
      </div>
    </footer>
