...
const translations = {
  ar: {
    ...
    subtitle: "قم بتعبئة النموذج أدناه لتقديم طلب استثمار",
    ...
    investmentOpportunity: "الفرصة الاستثمارية (اختياري)",
    selectOpportunity: "اختر فرصة استثمارية",
    general: "طلب عام - غير مرتبط بفرصة محددة",
  ...
  en: {
    ...
    subtitle: "Fill out the form below to submit an investment application",
    fullName: "Full Name",
    ...
    investmentOpportunity: "Investment Opportunity (optional)",
    selectOpportunity: "Select an opportunity",
    general: "General application - not linked to a specific opportunity",
        ...
        <div className="max-w-2xl mx-auto">
          ...
          >
            ...
            {investments.length > 0 && (
              <div>
                <label className="block text-sm font-bold text-gov-charcoal dark:text-white mb-2">
                  <Briefcase size={16} className="inline me-2" />
                  {t.investmentOpportunity}
                </label>
                <select
                  value={form.investment_id}
                  onChange={(e) =>
                    handleChange("investment_id", e.target.value)
                  }
                  className={inputClass("investment_id")}
                >
                  <option value="">{t.general}</option>
                  {investments.map((inv) => (
                    <option key={inv.id} value={inv.id}>
                      {language === "ar" ? inv.title_ar : inv.title_en}
                    </option>
                  ))}
                </select>
              </div>
