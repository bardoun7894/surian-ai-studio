...
const MegaMenu: React.FC<MegaMenuProps> = ({ label, sections, active, onMouseEnter, onMouseLeave, onClick }) => {
                    ...
                    >
                        <div className={`${isMultiSection ? 'flex divide-x rtl:divide-x-reverse divide-gray-100 dark:divide-white/5' : ''}`}>
                            {sections.map((section, idx) => (
                                <div key={idx} className={`pt-2.5 pb-4 ${isMultiSection ? "flex-1 min-w-[200px]" : ""}`}>
                                    {section.title && (
